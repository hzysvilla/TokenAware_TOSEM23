    ...
    with open(filename) as json_file:
        ...
        for u, v in c.iteritems():
            ...
            if len(current_file) == nsub:
                with open(
                        filename.split(".")[0] + "_" + str(file_index) +
                        '.json', 'w') as outfile:
        ...
        if len(current_file):
            with open(
                    filename.split(".")[0] + "_" + str(file_index) + '.json',
                    'w') as outfile:
                json.dump(current_file, outfile)
                current_file.clear()
...
def get_contract_stats(list_of_contracts):
    with open("concurr.csv", "w") as stats_file:
        ...
        fp.writerow([
            "Contract address", "No. of paths", "No. of concurrency pairs",
            "Balance", "No. of TXs", "Note"
        ])
...
def run_command(cmd):
    ...
    solc_p = subprocess.Popen(shlex.split(cmd),
