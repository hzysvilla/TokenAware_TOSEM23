		...
		for _, block := range blocks {
			for _, transaction := range block.Transactions() {
				if transaction.To() == nil {
					...
					//add by hzy on 20-9-26
					//To Get runtime bytecode from deployed bytecode with signature
					//var RuntimeByteCode string=d.GetRuntimeBytecodeBySignature(deployBytecode)
					//if len(RuntimeByteCode)==0{
					RuntimeByteCode:=hex.EncodeToString(d.GetRuntimeBytecodeByPreExec(transaction))
					//}
					if len(RuntimeByteCode)==0{
						RuntimeByteCode="6060"
					}
					//add end
					datalog.AddRuntimeCodeOnAfterDownload(txHash,RuntimeByteCode)
				}
			}
		}
		//fmt.Println("zwsdebuginfo","finished to get runtime code......")
		datalog.DWg.Done()
	}()
	//add end
...
//add by hzys
//To get rumtime bytecode
func (d *Downloader) GetRuntimeBytecodeByPreExec(transaction *types.Transaction) []byte {//use PreExec
	var test tests.VMTest
	var vmconfig vm.Config
	test.InitialEnv()
	test.Setexec(common.HexToAddress("0x2a5994b501E6A560e727b6C2DE5D856396aaDd38"),
		common.HexToAddress("0x2a5994b501E6A560e727b6C2DE5D856396aaDd38"),
		common.HexToAddress("0x2a5994b501E6A560e727b6C2DE5D856396aaDd38"),
		transaction.Data(),
		transaction.Value(),
		transaction.Gas(),
	...
	_,_,_,RuntimeByteCode:=test.Create(vmconfig,false)
	return RuntimeByteCode
}

func (d *Downloader) GetRuntimeBytecodeBySignature(Deployedbytecode string) string {//use signature
	if len(Deployedbytecode) < 4 {
		return "6060"
	}
	index := strings.Index(Deployedbytecode, "396000f300")
	if index != -1 {
		if len(Deployedbytecode) > index+10 {
			return Deployedbytecode[index+10:]
		}
	}
	index = strings.Index(Deployedbytecode, "396000f3"+Deployedbytecode[:4])
	if index != -1 {
		if len(Deployedbytecode) > index+8 {
			return Deployedbytecode[index+8:]

		}
	}
	if strings.Contains(Deployedbytecode, "396000f3") {
		index = strings.Index(Deployedbytecode, "00"+Deployedbytecode[:4])
		if index == -1 {
			index = strings.Index(Deployedbytecode, "006060")
			if index == -1 {
				index = strings.Index(Deployedbytecode, "006080")
			}
		}
		if index != -1 && len(Deployedbytecode) > index+2 {
			return Deployedbytecode[index+2:]
		}
	}
	if len(Deployedbytecode) > 8 && Deployedbytecode[len(Deployedbytecode)-8:] == "5b6000f3" {
		index = strings.Index(Deployedbytecode, "60003961")
		if index != -1 && len(Deployedbytecode) > index+14 {
			return  Deployedbytecode[index+14:]
		}
	}
	return ""
